...
class TestCircle:
    ...
    def test_circle_creation(self):
        ...
        assert circle.region_dimension == 2
    ...
    def test_contains_invalid_dimension(self):
        """Test that contains() rejects wrong dimensional points."""
        circle = Circle(center=[0, 0], radius=5.0)
        with pytest.raises(ValueError, match="Point must be 2D"):
            circle.contains([0, 0, 0])
...
class TestPolygon:
    ...
    def test_polygon_creation(self):
        ...
        assert polygon.region_dimension == 2
...
class TestRectangle:
    ...
    def test_rectangle_creation(self):
        ...
        assert rect.region_dimension == 2
...
class TestGeometricRegionBase:
    """Tests for the GeometricRegion base class behavior."""
    
    def test_dimension_enforcement(self):
        """Test that dimension enforcement works across all shapes."""
        circle = Circle(center=[0, 0], radius=5.0)
        
        # 1D point should fail
        with pytest.raises(ValueError, match="Point must be 2D"):
            circle.contains([0])
        
        # 3D point should fail
        with pytest.raises(ValueError, match="Point must be 2D"):
            circle.contains([0, 0, 0])
        
        # 2D point should work
        assert circle.contains([0, 0]) is True
    
    def test_region_dimension_attribute(self):
        """Test that all shapes have the correct region_dimension attribute."""
        circle = Circle(center=[0, 0], radius=5.0)
        polygon = Polygon(vertices=[[0, 0], [1, 0], [0, 1]])
        rect = Rectangle(vertices=[[0, 0], [1, 0], [1, 1], [0, 1]])
